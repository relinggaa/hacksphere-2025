...
class TiketBandaraController extends Controller
{
    public function index()
    {
        try {
            $tikets = TiketBandara::orderBy('id', 'desc')->get();
            
            return response()->json([
                'success' => true,
                'message' => 'Data tiket bandara berhasil dimuat',
                'data' => $tikets
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Gagal memuat data tiket bandara: ' . $e->getMessage()
            ], 500);
        }
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $tiket = TiketBandara::create($validator->validated());

            return response()->json([
                'success' => true,
                'message' => 'Data tiket bandara berhasil disimpan',
                'data' => $tiket
            ], 201);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Gagal menyimpan data tiket bandara: ' . $e->getMessage()
            ], 500);
        }
    }
    ...
    {
        try {
            $tiket = TiketBandara::findOrFail($id);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Data tiket bandara tidak ditemukan'
            ], 404);
        }
    }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => 'Data tiket bandara berhasil diupdate',
                'data' => $tiket
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Gagal mengupdate data tiket bandara: ' . $e->getMessage()
            ], 500);
        }
    }
    ...
    public function destroy($id)
    {
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => 'Data tiket bandara berhasil dihapus'
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Gagal menghapus data tiket bandara: ' . $e->getMessage()
            ], 500);
        }
    }
    ...
    {
        try {
            // Filter keretas yang mungkin untuk bandara
